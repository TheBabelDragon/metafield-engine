...
private:
    ...
    static void fill_contour(OccupancyBlob& b){
        ...
        for(int i=0;i<24;++i){
            const float t=float(i)/24.f*6.2831853f,ct=std::cos(t),st=std::sin(t),ca=std::cos(b.angle),sa=std::sin(b.angle);
    ...
    std::vector<OccupancyBlob> extract_raw(const CsiEstimate& e) const {
        ...
        for(int seed=0;seed<4;++seed){
            ...
            for(int z=1;z<gz-1;++z) for(int x=1;x<gw-1;++x){
                if(used[size_t(idx(x,z))]) continue;
                float v=e.grid[size_t(idx(x,z))];
            ...
            for(size_t qi=0;qi<q.size();++qi){
                auto [x,z]=q[qi]; float w=e.grid[size_t(idx(x,z))];
                float px=(float(x)+0.5f)/float(gw)*kWorld-kWorld*0.5f;
                float pz=(float(z)+0.5f)/float(gz)*kWorld-kWorld*0.5f;
                sx+=px*w; sz+=pz*w; sw+=w; sxx+=px*px*w; szz+=pz*pz*w; sxz+=px*pz*w;
                for(int dz=-1;dz<=1;++dz) for(int dx=-1;dx<=1;++dx){
                    if(!dx&&!dz) continue;
                    int nx=x+dx, nz=z+dz;
                    if(nx<0||nz<0||nx>=gw||nz>=gz) continue;
                    size_t ii=size_t(idx(nx,nz));
                    if(used[ii]||e.grid[ii]<rthr) continue;
                    used[ii]=1;
                    q.push_back({nx,nz});
                }
            }
    ...
    void match_tracks(const std::vector<OccupancyBlob>& raw,float motion){
        ...
        for(auto& t:tracks_){
            ...
            for(size_t i=0;i<raw.size();++i){
                if(taken[i]) continue;
                float dx=raw[i].x-t.blob.x, dz=raw[i].z-t.blob.z, d=std::sqrt(dx*dx+dz*dz);
                if(d<best_d){best_d=d;best=int(i);}
            }
            if(best>=0){
                taken[size_t(best)]=1; const auto& r=raw[size_t(best)];
        ...
        for(size_t i=0;i<raw.size();++i){
            if(taken[i]) continue;
            Track t; t.blob=raw[i]; t.blob.id=next_id_++; t.blob.motion=motion; t.blob.trail={t.blob.x,t.blob.z};
